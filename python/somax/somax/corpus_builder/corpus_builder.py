...
from somax.factor_oracle.multi_segmentation import slice_audio_by_feature
...
class CorpusBuilder:
    ...
    def build(self, filepath: str, corpus_name: Optional[str] = None, **kwargs) -> Corpus:
        ...
        if os.path.isdir(filepath):
            filepaths, content_type = self._folder_content(filepath)  #  List[str], Optional[Type[Corpus]]
    ...
    def _build_corpuses_with_multi_segmentation(self, filepaths: List[str], name: str,
                                               foreground_channels: Optional[List[int]] = None,
                                               background_channels: Optional[List[int]] = None,
                                               onset_channels: Optional[List[int]] = None,
                                               segmentation_mode: AudioSegmentation = AudioSegmentation.FEATURE,
                                               hop_length: int = 512,
                                               estimated_initial_bpm: float = 120.0,
                                               beat_tightness: float = 100.0,
                                               Dmax : List[int]= [0.2, 0.2],
                                               features_used: List[Type[CorpusFeature]] =[YinDiscretePitch, OnsetChroma],
                                               **kwargs) -> Tuple[Corpus, AudioMetadata]:
...
'''
filepaths = ["/Users/merlin/Documents/Tuto_4Players/Corpus_Tuto/Joelle.wav"]
name = "test_corpus"
Dmax = [0.1, 0.1]
features_used = [YinDiscretePitch, OnsetChroma]



corpus, metadata = CorpusBuilder()._build_audio(filepaths, name, foreground_channels=None, background_channels=None, onset_channels=None)
corpuses, metadatas = CorpusBuilder()._build_corpuses_with_multi_segmentation(filepaths, name, Dmax= Dmax, features_used = features_used)
print(corpuses, metadatas)
'''