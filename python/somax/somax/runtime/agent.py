...
from typing import Type, Any, Optional, List, Tuple, Type, Union, Dict
import mido
...
class OscAgent(Agent, AsyncioOscObject):
    ...
    def influence(self, path: str, feature_keyword: str, *value):
        if not self.scheduling_handler.running:
            return
        try:
            #print("INFLUENCE agent" , path, feature_keyword, value)
    ...
    def create_atom(self, path: str = "", weight: float = Atom.DEFAULT_WEIGHT, classifier: str = "",
        ...
        try:
            ...
            #print("ATOM CREATED", path_and_name, weight, classifier, activity_pattern, memory_space, self_influenced, enabled, override)
            self.send_atoms()
            self._send_eligibility()
    ...
    def set_region_mask(self, region_index: int, start_time: float, end_time: Optional[float]) -> None:
        try:
            ...
            self.player.update_region_mask_for_vmo()
    ...
    #########################  FO 
    '''       
    def set_FO(self,  state):
        self.player.set_factor_oracle(bool(state))

    def set_FO_fallback(self, state):
        self.player.set_factor_oracle_fallback(bool(state))

    def set_used_feature(self, feature_type: str):
        feature_type = self.feature_from_string([feature_type])[0]
        self.player.set_used_feature(feature_type)

    def set_guided_FO(self, state):
        self.player.set_oracle_guided(bool(state))

    def set_FO_navigation_mode(self, str):
        self.player.set_oracle_navigation_mode(str)

    def set_max_continuity(self, max_continuity: int):
        self.player.set_max_continuity(max_continuity)
    
    def create_FO(self, feature_used, max_continuity: int, navigation_mode : int,isguided: bool, window_size: int, temperature: float,
                  windowed_fallback_mode: int, behaviour_when_influenced: int,oracle_type: str, vmo_threshold= -1):
        print("CREATE FO oracle_type", oracle_type)
        print(navigation_mode)
        print(feature_used, "feature_used")
        #feature_types = self.feature_from_string(feature_types)
        feature_used = self.feature_from_string([feature_used])[0]
        self.player.create_FO(feature_used, max_continuity,navigation_mode,isguided, window_size,temperature, windowed_fallback_mode, 
                              behaviour_when_influenced,oracle_type, vmo_threshold)
        if vmo_threshold == -1 :
            self.send_IR_points()
            self._send_optimal_IR()

    def set_feature_used(self,feature_used):
        feature_used = self.feature_from_string([feature_used])[0]
        self.player.set_feature_used(feature_used)
        self._send_optimal_IR()
    
    
    #TODO it shouldn't be hardcoded + it's not well written
    def oracle_visualisation_PNG(self,ispitch:int, ischroma:int, ismfcc:int, isrms:int,iscentroid:int, lrs_threshold:int = 1):
        list_features = [ispitch, ischroma, ismfcc, isrms, iscentroid]
        print("agent", list_features)
        self.player.oracle_visualisation_PNG(list_features, lrs_threshold)
    
    #TODO it shouldn't be hardcoded + it's not well written
    def oracle_visualisation_omax(self,foldername):
        print("agent", foldername)
        foldername = foldername[10:]
        print("agent", foldername)
        self.player.oracle_visualisation_omax(foldername)

    def save_oracles(self, foldername):
        print("agent", foldername)
        foldername = foldername[10:]
        print("agent", foldername)
        self.player.save_oracles(foldername) 

    def feature_from_string(self, feature_types: list[str]) -> Type[CorpusFeature]:
    
        feature_mapping: Dict[str, Type[CorpusFeature]] = {
            "YinDiscretePitch": YinDiscretePitch,
            "Pitch": YinDiscretePitch,
            "OnsetChroma": OnsetChroma,
            "Chroma"    : OnsetChroma, 
            "MFCC": OnsetMFCCs,
        ...
        try:
            return [feature_mapping[feature_type] for feature_type in feature_types]
        except KeyError:
            raise ValueError(f"Unrecognized feature type: {feature_types}")
    ...
    def send_IR_points(self):
        dict_IR_points = self.player.get_IR_points()
        print("SEND IR POINTS all", dict_IR_points)
        for keys in dict_IR_points:
            if dict_IR_points[keys] is not None:
                print("SEND IR POINTS", keys, dict_IR_points[keys], len(dict_IR_points[keys]))
                feature = self.string_from_feature(keys)
                self.target.send( ["IR_points", feature], dict_IR_points[keys])
    
    def _send_optimal_IR(self):
        print("SEND OPTIMAL IR")
        print(self.player)
        if not self.player.get_oracle_info():
            self.target.send("Optimal_IR", self.player.get_oracle_info()[0])
    
    def _send_optimal_IR_with_sym_constraint(self):
        print("SEND OPTIMAL IR WITH SYM CONSTRAINT")
        print(self.player.get_oracle_info())
        self.target.send("Optimal_IR_sym_constraint", self.player.get_oracle_info()[1])

    
    def get_info_IR(self,  threshold, feature):
        print("GET INFO IR", feature, threshold)
        
        infos_keys  = ["current_nb_cluster", "current_IR", "optimal_threshold", "optimal_nb_cluster", "optimal_IR"]
        feature = self.feature_from_string([feature])[0]
        infos_content = self.player.get_IR_infos(feature, threshold)
        for key, content in zip(infos_keys, infos_content):
            self.target.send( ["IR_info", key], content)
    
    
    def send_jump_infos(self):
        if self.player.get_jump_infos() == None:
            return None, None
        infos_last, infos_next, infos_transpo = self.player.get_jump_infos()
        for keys in infos_last:
            self.target.send( ["last", keys], infos_last[keys])
        for keys in infos_next:
            self.target.send( ["next", keys], infos_next[keys])
        for keys in infos_transpo:
            self.target.send( ["transpo", keys], infos_transpo[keys])

    def get_jump_context_length(self):
        self.target.send("jump_context_length",self.player.get_jump_context_length())
        return self.player.get_jump_context_length()

    def evaluate_oracle(self,num_step=None): 
        if num_step < 1:
            num_step =  None
        return self.player.evaluate_oracle(num_step)
    
    
    def set_transpo_threshold(self, threshold: int):
        self.player.set_transpo_threshold(threshold)


    def set_softmax_temperature(self, temperature: float):
        self.player.set_softmax_temperature(temperature)

    def set_windowed_fallback_mode(self, mode: str):
        self.player.set_windowed_fallback_mode(mode)

    def set_editing_window_size(self, size: int):
        self.player.set_editing_window_size(size)

    def set_behaviour_when_influenced(self, behaviour: str):
        self.player.set_behaviour_when_influenced(behaviour)

    def add_transposition_to_FO(self, semitones: int, **kwargs):
        self.player.add_transposition_to_FO(semitones, **kwargs)
    
    def remove_transposition_from_FO(self, semitones: int, **kwargs):
        self.player.remove_transposition_from_FO(semitones, **kwargs)
        
    def set_vmo_threshold(self, threshold: float, sym_min=0, sym_max=0):
        self.player.set_vmo_threshold(threshold, sym_min, sym_max)
        if sym_max > 0 and sym_max >= sym_min:
            print("SEND OPTIMAL IR WITH SYM CONSTRAINT")
            self._send_optimal_IR_with_sym_constraint()
        
    
    def enable_transpo(self, state : int):
        self.player.enable_transpo(state)

    def set_taboo_length(self, n: int):
        self.player.set_taboo_length(n)
    
    def enforce_taboo_FO(self, state: int):
        self.player.enforce_taboo(state)
    
    def next_local_ir_max(self,threshold: float):
        local_ir_max = self.player.near_local_ir_max(1,threshold)
        self.target.send( ["local_ir_max"], local_ir_max)
    def previous_local_ir_max(self,threshold: float):
        local_ir_max = self.player.near_local_ir_max(-1,threshold)
        self.target.send( ["local_ir_max"], local_ir_max)
'''
